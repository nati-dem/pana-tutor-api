...
export class AuthRouter {
  ...
  registerRouter = router.post(
    "/register",
    asyncHandler(async (req, res, next) => {
      // TODO - add request payload validation
      const reqObj = req.body as UserSignupRequest;
      if (
        isEmpty(reqObj.password) ||
        isEmpty(reqObj.username) ||
        isEmpty(reqObj.email) ||
        isEmpty(reqObj.name)
      ) {
        throw new AppError(
          400,
  ...
  tokenValidationRouter = router.post(
    "/token-validate",
    asyncHandler(async (req, res, next) => {
  ...
  googleLogin = router.get(
    "/google",
    passport.authenticate("google", { scope: ["profile"] })
  );

  googleCallback = router.get(
    "/google/callback",
