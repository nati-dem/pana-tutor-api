...
export class UserDAO extends BaseDAO {
  ...
  saveUser = async (req: UserSignupRequest) => {
    const phone = req.meta && req.meta.phone_number.length > 0 ? req.meta.phone_number[0] : '';
    const query = `INSERT INTO users (user_id, name, email, phone, address)
      VALUES (?, ?, ?,?,?) `;
    const params = [req.id, req.name, req.email, phone, ''];
