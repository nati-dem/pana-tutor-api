...
export class AuthService {
  ...
  authenticate = async (loginRequest: UserLoginRequest) => {
    return await this.apiExecuter.doPost(
      loginRequest,
  ...
  isTokenValid = async (token: string, userId): Promise<boolean> => {
    const tokenInCache = this.appCache.get(
      AppConstant.USER_TOKEN_KEY + "_" + userId
    );
    if (tokenInCache && token === tokenInCache) {
      return this.isTokenUnexpired(tokenInCache);
    } else {
      const tokenResp = await this.apiExecuter.doPost(
        {},
        AppConstant.TOKEN_VALIDATION_URL,
        false,
        token
      );
